...
class HealCombatantsAction(Action):
    def __init__(self):
        super().__init__(7, "Heal Combatants")
    ...
    def process(self, ui, encounter):
        if encounter.combatants:
            process_heal_combatants(ui, encounter)
        else:
...
class MeleeManager:
    ...
    def __init__(self):
        ...
        self.actions = [
            ...
            NextEncounterAction(),
            NextAttackAction(),
            HealCombatantsAction(),
            SetInitiativeAction(),
            ListCombatantsInformation(),
            QuitAction(),
        ]

        # order actions by their numeric code so prompts display sorted
        self.actions.sort(key=lambda a: a.code)
        self.actions_map = {action.code: action for action in self.actions}
...
def get_input(
    ui,
    action_prompt,
    response_option=EXIT_TO_MENU,
    response_exception=ExitToMenuException,
) -> str:
...
def get_numeric_input(
    ...
    return ui.get_numeric_input(
        action_prompt, response_prefix, response_option, response_exception
    )
...
def find_next_defender(ui, encounter, attacker) -> cm1.Combatant:
    ...
    while defender_typeabbrseq == "":
        ...
        if len(defender_typeabbrseq := get_input(ui, message)) == 0:
            ...
            ui.output(
                f"{ui.INDENT_LEVEL_03}{attacker.typeabbrseq} cannot self-attack. Try again"
            )
            defender_typeabbrseq = ""
            continue
            ...
            if encounter.is_combatant(defender_typeabbrseq) == False:
                ui.output(
                    f"{ui.INDENT_LEVEL_03}Defender {defender_typeabbrseq} does not exist. Try again"
                )
                defender_typeabbrseq = ""
                continue
                ...
                if defender.combattype == attacker.combattype:
                    if (
                        len(
                            get_input(
                                ui,
                                f"{ui.INDENT_LEVEL_03}{attacker.typeabbrseq} and {defender.typeabbrseq} are both friends. Are you sure? (<Enter> for No, y for Yes) ",
                            )
                        )
                        == 0
                    ):
        ...
        # all combatants checked and defender not found (still alive but not in combatant list)
        if defender_count == len(encounter.combatants) and defender_typeabbrseq == "":
            raise Exception(
                f"ERROR occurred during Attacker {attacker.typeabbrseq}'s attack. Defender unknown. (find_next_defender)"
            )

        # previous defender not found in combatant list because someone else kill them but they were still set as attacker's defender
        print(
            f"{ui.INDENT_LEVEL_03}Defender {defender_typeabbrseq} not found. Try again"
        )
        attacker.defender_typeabbrseq = ""
        defender_typeabbrseq = ""
    ...
    for combatant in encounter.combatants:
        ...
        if combatant.is_player_character():
            ...
            if combatant.initiative >= encounter.INITIATIVE_ACTIVE_MINIMUM:
                initiative_reroll = get_input(
                    ui,
                    f"{ui.INDENT_LEVEL_01}Re-roll {combatant.typeabbrseq}'s {combatant.initiative} initiative? (<Enter> for No, A for Auto-roll, M for Manual-entry) ",
                )
            ...
            ui.output(
                f"{ui.INDENT_LEVEL_01}{combatant.typeabbrseq}'s initiative set to {combatant.initiative}"
            )

    encounter.check_duplicate_initiative()
    for combatant in encounter.combatants:
        encounter.combat_data.log_initiative(
            encounter.encounter,
            encounter.round,
...
def get_combatant_initiative(ui, encounter, combatant) -> None:
    ...
    while True:
        if (
            len(initiative_raw := get_input(ui, initiative_prompt)) == 0
        ):  # Use previous value?
            if combatant.initiative:  # non-zero previous initiative value exists
                initiative = combatant.initiative
                ui.output(
                    f"{ui.INDENT_LEVEL_02}Keeping previous initiative value of {combatant.initiative}"
                )
                return
        ...
        if not initiative_raw.isnumeric():  # check if initiative is not numeric
            ui.output(
                f"{ui.INDENT_LEVEL_02}Initiative value of {initiative} is not a number"
            )
            continue

        initiative = int(initiative_raw)
        if (
            initiative < encounter.INITIATIVE_MINIMUM
        ):  # check if initiative is less than minimum
            ui.output(
                f"{ui.INDENT_LEVEL_02}An initiative value of {initiative} is less than {encounter.INITIATIVE_MINIMUM}"
            )
            continue

        if (
            initiative > encounter.INITIATIVE_MAXIMUM
        ):  # check if initiative is greater than maximum
            ui.output(
                f"{ui.INDENT_LEVEL_02}An initiative value of {initiative} is more than {encounter.INITIATIVE_MAXIMUM}"
            )
            continue

        if (
            initiative < encounter.INITIATIVE_ACTIVE_MINIMUM
        ):  # check if initiative is less than active minimum
            if len(
                get_input(
                    ui,
                    f"{ui.INDENT_LEVEL_03}Initiative value {initiative} is for inactive combatants. Keep it? (<Enter> for Yes, N for No) ",
                )
            ):
                continue
...
def get_defenders(ui, encounter, attacker) -> list:
    ...
    while len(special_attack_defenders_raw) == 0:
        if (
            len(
                special_attack_defenders_raw := get_input(
                    ui, special_attack_defenders_message
                )
            )
            == 0
        ):
    ...
    special_attack_groups = [
        group[1:] for group in special_attack_defenders if group[0:1] == "#"
    ]
    ...
    [
        ...
        for combatant in encounter.combatants
        if (
            has_common_substring_elements(
                combatant.group.split(","), special_attack_groups
            )
        )
    ...
    special_attack_defenders = [
        defender
        for defender in special_attack_defenders
        if defender != attacker.typeabbrseq
    ]

    # get defenders (ignoring groups)
    [
        defenders.append(combatant)
        for combatant in encounter.combatants
        if combatant.typeabbrseq in special_attack_defenders
    ]
...
def get_to_hit_roll(ui, encounter, combatant) -> int:
    ...
    message += (
        f"{encounter.TO_HIT_DIE_MINIMUM}-{encounter.TO_HIT_DIE_MAXIMUM} manual entry) "
    )
    to_hit_input = ""
    ...
    while to_hit_input == "":
        ...
        if (
            to_hit_roll < encounter.TO_HIT_DIE_MINIMUM
            or to_hit_roll > encounter.TO_HIT_DIE_MAXIMUM
        ):
            ui.output(
                f"{ui.INDENT_LEVEL_03}'To Hit' roll must be between {encounter.TO_HIT_DIE_MINIMUM} and {encounter.TO_HIT_DIE_MAXIMUM}. Entered {to_hit_roll} value."
            )
            to_hit_input = ""
            continue
...
def log_hit_action(
    encounter: cm1.Encounter,
    attacker: cm1.Combatant,
    defender: cm1.Combatant,
    damage: int,
    isdefenderdamaged: bool,
    xp_total: int,
    xp_earned: int,
    message: str,
):
    ...
    """
    encounter.combat_data.log_action(
        encounter.encounter,
        encounter.round,
        attacker.combattype,
        attacker.abbr,
        ...
        attacker.group,
        attacker.initiative,
        encounter.combatant_attack_number,
        defender.combattype,
        defender.abbr,
        defender.seq,
        defender.group,
    ...
    if isdefenderdamaged:
        encounter.combat_data.update_combatant_hit_points(
            defender.combattype,
            defender.abbr,
            defender.seq,
            defender.hpmax,
            defender.hp,
        )  # update db with new post-damage hp
    else:
        encounter.combat_data.update_combatant_hit_points(
            attacker.combattype,
            attacker.abbr,
...
def process_attack_sequence(ui, encounter) -> bool:
    ...
    ui.output(
        f"{encounter.format_encounter()} | {encounter.format_attack_type()} Attacks"
    )
    ui.output(encounter.format_combatants())
        ...
        if len(get_input(ui, message)) > 0:
            ...
            if len(status := get_input(ui, status_prompt)) == 0:
                ...
                if attacker.initiative >= encounter.INITIATIVE_ACTIVE_MINIMUM:
                    ...
                    ui.output(
                        f"{ui.INDENT_LEVEL_02}{attacker.typeabbrseq} will be able to attack next round"
                    )
                else:
                    ui.output(
                        f"{ui.INDENT_LEVEL_02}{attacker.typeabbrseq} will remain inactive"
                    )
    ...
    ui.output(
        f"{ui.INDENT_LEVEL_02}{encounter.format_attack_type()} Attack #{encounter.combatant_attack_number}"
    )
    ...
    if (
        len(
            ui.get_input(
                f"{ui.INDENT_LEVEL_02}Attack type: ([Enter] for regular, S for special) "
            )
        )
        == 0
    ):
    ...
    if (
        len(
            get_input(
                ui, f"{ui.INDENT_LEVEL_02}Attack again? (<Enter> for No, y for Yes) "
            )
        )
        == 0
    ):
...
def process_attack_regular(ui, encounter, attacker) -> None:
    ...
    if (
        ...
        log_hit_action(
            ...
            True,
            defender.xp,
            encounter.calculate_earned_xp(
                defender.hpmax, defender.hp, damage, defender.xp
            ),
            message,
        )
        ui.output(
            f"{ui.INDENT_LEVEL_03}{message} {defender.typeabbrseq} for {damage} points damage ({defender.hp} remaining)"
        )
    ...
    if (
        len(
            get_input(
                ui,
                f"{ui.INDENT_LEVEL_04}Is attack fumbled/cursed? ([Enter] for No, Y for Yes) ",
            )
        )
        == 0
    ):
    ...
    penalty_xp = 0
    damage = get_numeric_input(
        ui, f"{ui.INDENT_LEVEL_05}Enter {attacker.typeabbrseq} fumble damage: "
    )
    if attacker.combattype == encounter.COMBATTYPE_FRIEND:
        penalty_xp = get_numeric_input(
            ui, f"{ui.INDENT_LEVEL_05}Enter penalty xp (-number): "
        )
...
def process_attack_special(ui, encounter, attacker) -> None:
    ...
    if (
        len(
            get_input(
                ui,
                f"{ui.INDENT_LEVEL_03}Is saving throw allowed? ([Enter] for Yes, N for No) ",
            )
        )
        == 0
    ):
        ...
        if (
            len(
                get_input(
                    ui,
                    f"{ui.INDENT_LEVEL_03}If save successful, defender takes no damage or 50% damage? ([Enter] for None, Y for 50%) ",
                )
            )
            > 0
        ):
        ...
        while saving_throw_type == 0:
            saving_throw_type_raw = get_numeric_input(
                ui,
                f"{ui.INDENT_LEVEL_04}Select saving throw type: ",
                ui.INDENT_LEVEL_04,
            )
            if (saving_throw_type_raw > 0) and (
                saving_throw_type_raw <= len(cm1.Saving_Throw.SAVING_THROW_TYPE)
            ):
    ...
    if len(defender_list) > 1:
        ...
        if (
            len(
                get_input(
                    ui,
                    f"{ui.INDENT_LEVEL_02}Enter damage for each defender? [Enter for No, Y for Yes] ",
                )
            )
            == 0
        ):
    ...
    for index, defender in enumerate(defenders, 1):
        ...
        if saving_throw_permitted == True:
            saving_throw = encounter.get_saving_throw(
                defender.savingthrowclasstype,
                defender.savingthrowlevel,
                defender.savingthrowlevelpdm,
                cm1.Saving_Throw.SAVING_THROW_TYPE[saving_throw_type],
            )
            if defender.is_player_character():
                saving_throw_value = 0
                while saving_throw_value == 0:
                    saving_throw_value_raw = get_numeric_input(
                        ui,
                        f"{ui.INDENT_LEVEL_03}Enter {defender.typeabbrseq}'s save vs. {cm1.Saving_Throw.SAVING_THROW_TYPE[saving_throw_type]}: ({saving_throw} needed) ",
                        ui.INDENT_LEVEL_04,
                    )
                    if (saving_throw_value_raw > 0) and (
                        saving_throw_value_raw <= encounter.TO_HIT_DIE
                    ):
        ...
        if (is_damage_variable == True) or (index == 1):
            damage_base = ui.get_numeric_input(
                f"{ui.INDENT_LEVEL_03}Enter special attack damage (+/-number): "
            )
        ...
        if (
            attacker.combattype == encounter.COMBATTYPE_FRIEND
            and defender.combattype == encounter.COMBATTYPE_FRIEND
        ):
            ...
            # Calculate xp
            earned_xp_base = encounter.calculate_earned_xp(
                defender.hpmax, defender.hp, damage, defender.xp
            )

        earned_xp = int(earned_xp_base * saving_throw_modifier)
        log_hit_action(
            encounter, attacker, defender, damage, True, defender.xp, earned_xp, message
        )
        ui.output(
            ui.INDENT_LEVEL_04
            + output_message.replace("~~", str(damage))
            + f" points damage ({defender.hp} remaining)\n"
        )

    return


def process_heal_combatants(ui, encounter) -> None:
    """heal combatants in encounter

    args:
        ui: user interface
        encounter: current Encounter
    """
    message = "Who is the healer? "
    healer = get_input(ui, message)
    while encounter.is_combatant(healer) == False:
        ui.output(f"{ui.INDENT_LEVEL_01}Healer {healer} does not exist. Try again")
        healer = get_input(ui, message)

    message = "Who is to be healed? "
    healee = get_input(ui, message)
    while encounter.is_combatant(healee) == False:
        ui.output(f"{ui.INDENT_LEVEL_01}Healee {healee} does not exist. Try again")
        healee = get_input(ui, message)

    message = "Healing method? "
    healing_method = get_input(ui, message)
    while len(healing_method) == 0:
        ui.output(f"{ui.INDENT_LEVEL_01}Healing method cannot be empty. Try again")
        healing_method = get_input(ui, message)

    message = f"How many hit points to heal {healee}? "
    heal_points = get_numeric_input(ui, message)

    message = f"How many experience points for healing? "
    heal_xp = get_numeric_input(ui, message)

    healer_combatant = None
    for combatant in encounter.combatants:
        if combatant.typeabbrseq == healer:
            healer_combatant = combatant
            break

    healee_combatant = None
    for combatant in encounter.combatants:
        if combatant.typeabbrseq == healee:
            healee_combatant = combatant
            break
    ...
    log_hit_action(
        ...
        0,
        0,
        healing_method + " BEFORE HEALING",
    )

    healee_combatant.take_damage(-heal_points)
    log_hit_action(
        encounter,
        healer_combatant,
        healee_combatant,
        ...
        0,
        heal_xp,
        healing_method + " AFTER HEALING",
    )

    message = f"{ui.INDENT_LEVEL_01}{healer_combatant.typeabbrseq} healed {healee_combatant.typeabbrseq} for {heal_points} hit points ({healee_combatant.hp} remaining) and earned {heal_xp} xp"
    ui.output(message)
...
def process_round(ui, encounter) -> None:
    ...
    while True:
        ...
        encounter.ismissileattack = (
            len(
                get_input(
                    ui, f"Is round missile or melee? [Enter] for missile, m for melee "
                )
            )
            == 0
        )
            ...
            if encounter.foe_count == 0:
                if (
                    len(
                        get_input(
                            ui,
                            f"{ui.INDENT_LEVEL_01}Encounter: {encounter.encounter} Round: {encounter.round} has no FOES. Continue? ([Enter] for No, y for Yes) ",
                        )
                    )
                    == 0
                ):
        ...
        if (
            len(get_input(ui, f"\nBegin next encounter? (<Enter> for Yes, N for No) "))
            == 0
        ):
            encounter.prepare_next_encounter()
...
def process_set_initiative(ui, encounter) -> None:
    ...
    while len(raw_initiative) == 0:
        ...
        if (int(raw_initiative) < encounter.INITIATIVE_MINIMUM) or (
            int(raw_initiative) > encounter.INITIATIVE_MAXIMUM
        ):
            ui.output(
                f"{ui.INDENT_LEVEL_01}Initiative must be between {encounter.INITIATIVE_MINIMUM} and {encounter.INITIATIVE_MAXIMUM}. Try again"
            )
            raw_initiative = ""
            continue
